...
const Card = ({ children, className = "" }) => (
  <div
    className={`rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all duration-300 ${className}`}
  >
    {children}
  </div>
);
    ...
    <div className="space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm font-medium text-slate-500">
            Chào mừng trở lại! Dưới đây là tình trạng hệ thống hiện tại.
          </p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {stats.map((stat, i) => (
          <Card
            key={i}
            className="relative overflow-hidden group hover:shadow-xl hover:shadow-slate-200/50 hover:-translate-y-1"
          >
            ...
            {stat.pulse && stat.value > 0 && (
              <div className="absolute top-0 right-0 h-1 w-full bg-indigo-500/10">
                <div
                  className="h-full bg-indigo-500 animate-[shimmer_2s_infinite]"
                  style={{ width: "30%" }}
                ></div>
              </div>
            )}
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Link to="/pois" className="group">
          <Card className="h-full hover:border-slate-300 hover:shadow-lg">
            ...
            <p className="mt-3 text-xs text-slate-500 line-clamp-2">
              Điều chỉnh thông tin địa danh, tọa độ và quản lý mã QR bảo mật.
            </p>
          </Card>
        </Link>
        ...
        <Link to="/users" className="group">
          <Card className="h-full hover:border-slate-300 hover:shadow-lg">
            ...
            <p className="mt-3 text-xs text-slate-500 line-clamp-2">
              Quản lý đặc quyền tài khoản, mở khóa zone pass và lịch sử quét.
            </p>
          </Card>
        </Link>
      </div>
